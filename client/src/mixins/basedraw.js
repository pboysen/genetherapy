...
function makeStage(drawing, easeljs, canvasId) {
  widget = drawing;
  stage = new easeljs.Stage(canvasId);
  stage.enableMouseOver();
  stage.cursor = "default";
  easeljs.Touch.enable(stage);
  drawing.stage = stage;
  back = new easeljs.Bitmap(drawing.image);
...
function run(easeljs) {
  easeljs.Ticker.on("tick", () => {
    stage.update();
  });
}
